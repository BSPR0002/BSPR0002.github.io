var Activity={
	"ReleaseAgreement":function() {
		window_board.display(HtmlArray.decoder([
			"您好，欢迎访问蓝天信息工厂！",["BR"],
		...
		"interface":function() {
			switch (localStorage.getItem("MobileDemo")) {
				...
				case "1":
					...
					break;
				default:
					window_board.display(HtmlArray.decoder(Activity.mobile_survey.HA,"Activity.mobile_survey"),"调查")
			};
		},
		...
		if (getNotificationPermission()!=0&&Data[0]) {
			...
			if ((LogManager(data.ID,data.name)||data.force==true)&&data.unshow!=true) {
				...
				if (data.board) model.click=function(){
					window.focus();
					window_board.display(HtmlArray.decoder(data.board,"News_"+data.ID),data.title)
					this.close();
				};
