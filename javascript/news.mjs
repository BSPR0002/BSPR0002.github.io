...
import { requestPermission } from "/component/web_permission_screen/web_permissions.mjs";
...
function showDetail(id, title, content) {
	try {
		new OverlayWindow(typeof content == "string" ? content : parse(content), title, { size: { width: "25rem" } }).addEventListener("show", function () { log[id] = Date.now() });
	} catch (none) {
		new OverlayWindow("出了点问题，内容出不来了……😢", "出错了", { size: { width: "25rem" } });
	}
}
...
function createWindow(content) { return new OverlayWindow(content, "推送通知", { size: { width: "25rem", height: "100%" } }) }
async function showBoard() {
	...
	container.addEventListener("click", function (event) {
		if (event.isTrusted && event.target != container) {
			overlayWindow.close();
			overlayWindow = createWindow(frame);
		}
	});
	overlayWindow = createWindow(frame);
